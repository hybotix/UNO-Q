...
Controls:
  ...
  S        — skip frame (leave blank) and advance
  Enter    — advance without changing label
  B        — go back one frame
  Q        — quit and save
...
def display_frame(row: dict, current: int, total: int, suggestion: str):
    ...
    print()
    print(f"  A)ccept '{suggestion}'  S)kip  B)ack  Q)uit  or type label: ", end="", flush=True)
...
def main():
    print("═" * 60)
    print("  VL53L5CX Data Visualizer  v1.0.3")
    print("  Hybrid RobotiX — HybX Development System")
    print("═" * 60)
    print()
    print("  A)ccept suggested label   S)kip frame (blank label)")
    print("  B)ack one frame           Q)uit and save")
    print("  Enter                     Advance without changing label")
    ...
    if current == total:
        ...
        print()
        print("  R) Re-label from beginning")
        print("  C) Continue browsing from end")
        print("  Q) Quit")
        print()
        while True:
            choice = input("  Choice [R/C/Q]: ").strip().upper()
            if choice == "Q":
                sys.exit(0)
            elif choice == "R":
                current = 0
    ...
    try:
        while 0 <= current < total:
            ...
            if key == "Q":
                ...
                # Enter with no input — advance without changing label
                current += 1
